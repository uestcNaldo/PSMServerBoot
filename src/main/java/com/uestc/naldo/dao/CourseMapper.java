...
public interface CourseMapper {
    ...
    @Update("UPDATE course SET pc_name=#{name}, pc_inst=#{inst}, pc_condition=#{condition}, pc_duration=#{duration}, pc_content=#{content}, pc_prise=#{prise}, pc_note=#{note} WHERE pcid=#{id}")
    ...
    @Select("SELECT * FROM course WHERE pc_name like CONCAT(CONCAT('%',#{name}),'%') ORDER BY pcid")
    ...
    @Select("SELECT * FROM course WHERE pcid=#{id}")
    @Results(id = "CourseById",value = {
            @Result(property = "id", column = "pcid",javaType = Long.class),
            @Result(property = "name", column = "pc_name", javaType = String.class),
            @Result(property = "inst", column = "pc_inst", javaType = String.class),
            @Result(property = "condition", column = "pc_condition", javaType = String.class),
